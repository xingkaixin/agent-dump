...
An experimental Rust implementation is being developed in [`rust/`](rust/README.md). It currently supports discovery, transcript assembly, and single-session export for all ten Providers. Cursor supports JSON/print; DeepChat, Cherry Studio, and MiniMax support JSON/Markdown/print; the other six also support raw. `--list` now lists all available Providers; `-q provider:codex,opencode` narrows discovery before scanning, and one failed Provider does not block the others.

URI validation, missing-session and unsupported-format failures have localized diagnostic blocks; lookup warnings go to stderr and final diagnostics to stdout. DeepChat, Cherry Studio, and MiniMax now preserve localized schema, missing-source, and migration diagnostics. The remaining seven Providers preserve missing-source diagnostics; Kimi raw export uses the file recorded during session lookup. JSONL and legacy SQLite record warnings are localized on stderr while readable content continues to export. Codex and Claude recover from unreadable title indexes with localized warnings and fallback titles. Codex, Claude and Pi report record conversion failures and continue exporting readable messages. The four file Providers and OpenCode/ZCode use current configuration until a source is selected, then keep it for the lifetime of the instance. Codex title indexes follow current configuration. DeepChat, Cherry and MiniMax reselect sources for each discovery or lookup; Cursor also uses current configuration for transcript reads. Full Provider and workflow parity is still pending; pip/npm install the Python implementation. See the [migration plan](docs/rust-migration-plan.md) for scope and acceptance criteria.
