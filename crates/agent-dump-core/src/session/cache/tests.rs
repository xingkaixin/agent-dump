use super::*;
use crate::providers::contract::{Discovery, Lookup};
...
impl<F: Fn(&Session) -> crate::Result<SessionData> + Send + Sync> Provider
    for Reader<F>
{
    fn discover(
        &mut self,
...
fn concurrent_leases_coalesce_isolate_and_release_payloads() {
    ...
    std::thread::scope(|scope| {
        for index in 0..4 {
            let (provider, session, cache, ready) =
                (&provider, &session, &cache, &ready);
            scope.spawn(move || {
                let mut lease = cache
...
fn source_and_session_signals_invalidate_cached_payloads() {
    ...
    for step in 0..6 {
        match step {
            ...
            2 => std::fs::write(&session.source_path, "longer replacement")
                .unwrap(),
...
fn failed_reads_retry_and_preserve_localized_error_identity() {
    ...
    let provider = Reader(|session: &Session| {
        if reads.fetch_add(1, Ordering::SeqCst) == 0 {
            return Err(crate::providers::error::ProviderError::Message([
                "unavailable",
                "不可用",
            ])
            .into());
    ...
    let error = cache
        ...
        .unwrap();
    assert_eq!(
        crate::providers::error::operation_message(error.as_ref(), true),
        "ValueError: 不可用"
    );
    assert_eq!(
        cache
...
fn provider_change_sources_include_payload_files_and_wal_only() {
    ...
    for registration in crate::providers::registry::all() {
        ...
        if registration.info.name == "kimi" {
            ...
            std::fs::write(session.source_path.join("metadata.json"), "{}")
                .unwrap();
            ...
            std::fs::write(session.source_path.join("context.jsonl"), "")
                .unwrap();
            assert_ne!(
                wire_signal,
                SessionSignal::new(provider.as_ref(), &session)
            );
            assert_eq!(
...
fn sqlite_wal_updates_invalidate_without_changing_database_bytes() {
    ...
    let provider = crate::providers::sqlite::SqliteProvider::open(
    ...
    let first = cache
        ...
        .unwrap();
    assert_eq!(
        serde_json::to_value(first.as_ref()).unwrap()["messages"][0]["parts"]
            [0]["text"],
        "First"
    );
    connection
    ...
    let second = cache
        ...
        .unwrap();
    assert_eq!(
        serde_json::to_value(second.as_ref()).unwrap()["messages"][0]["parts"]
            [0]["text"],
        "Second"
    );
