use crate::compat::value::{field, parsed_string, string};
...
pub fn injected_context(content: &Value) -> bool {
    let Some(items) = content.as_array().filter(|items| !items.is_empty())
    else {
        return false;
    };
    if items
...
pub fn subagent_notification(
    ...
    let payload: Value = crate::compat::json::from_str(body.trim()).ok()?;
...
pub fn record_subagent_output(
    ...
    let prompt = match arguments {
        ...
        _ => crate::compat::value::pretty_json(arguments),
    };
...
pub fn json_payload(data: &SessionData) -> SessionData {
    ...
    for message in &mut payload.messages {
        ...
        let Some(name) =
            inner_tag(body, "name").filter(|name| !name.is_empty())
        else {
            continue;
        };
    ...
    payload.messages.retain(|message| {
        message.role != "developer" && !message.parts.is_empty()
    });
    payload
}
...
pub fn output_parts(raw: &Value, timestamp: i64, custom: bool) -> Vec<Part> {
    ...
    let value = match parsed.as_ref() {
        Some(value) if value.is_object() && value.get("output").is_some() => {
            &value["output"]
        }
        Some(value) => value,
        None => raw,
    ...
    let text = match value {
        Value::Object(_) | Value::Array(_) => {
            crate::compat::value::pretty_json(value)
        }
        _ => string(value),
    };
