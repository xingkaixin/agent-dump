...
use crate::providers::contract::{
    DiagnosticSink, Provider, RecoverableDiagnostic,
};
...
impl Codex {
    ...
    fn title(
        ...
        if self.titles.is_none() {
            ...
            let index =
                self.roots.configured_root()?.join("session_index.jsonl");
            if index.exists()
                && let Err(error) =
                    jsonl::scan(&index, &mut |_| Ok(()), |record, _| {
                        let id = text(&record["id"]);
                        if !id.trim().is_empty()
                            && let Some(title) =
                                normalize_title(text(&record["thread_name"]))
                        {
            ...
            {
                diagnostics(RecoverableDiagnostic::TitleCacheFailed(
                    error.to_string(),
                ))?;
            }
    ...
    fn parse(
        ...
        if !payload.is_null() && !payload.is_object() {
            return Err(format!(
                ...
                crate::compat::value::type_name(payload)
            )
            .into());
        }
        ...
        let title = explicit_title
            .or(message_title)
            .or_else(|| {
                crate::providers::title::value_basename(&payload["cwd"])
            })
            .or_else(|| {
                path.parent()
        ...
        for record in scan.records.iter().chain(scan.tail.iter()) {
            ...
            if p["type"].is_array() || p["type"].is_object() {
                return Err(format!(
                    "unhashable type: '{}'",
                    crate::compat::value::type_name(&p["type"])
                )
                .into());
            }
            if matches!(
            ...
            if model.is_empty() {
                text(&p["model"]).trim().clone_into(&mut model);
                if model.is_empty() {
                    text(&p["arguments"]["model"])
                        .trim()
        ...
        if model.is_empty() {
            text(&payload["model_provider"]).clone_into(&mut model);
        }
        Ok(Some(Session {
            id,
            title,
...
impl Provider for Codex {
    ...
    fn read(
        ...
        transcript::read(session, zh, diagnostics)
    }

    fn json_payload(&self, data: &SessionData) -> serde_json::Value {
        serde_json::to_value(enrichment::json_payload(data)).unwrap()
    }
    ...
    fn title_index_follows_configuration_while_session_root_stays_selected() {
        ...
        for (root, title) in [(&first, "First"), (&second, "Second")] {
            ...
            std::fs::write(
                ...
                serde_json::json!({"id":"kept", "thread_name":title})
                    .to_string(),
            )
            .unwrap();
        }
