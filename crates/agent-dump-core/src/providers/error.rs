...
impl ProviderError {
    ...
    pub fn summary(&self, zh: bool) -> &str {
        match self {
            ...
            Self::Message(summary) | Self::Diagnostic { summary, .. } => {
                summary[usize::from(zh)]
            }
        }
    }
}
...
pub fn kind(error: &(dyn std::error::Error + 'static)) -> Option<&'static str> {
    ...
    match error.downcast_ref::<ProviderError>() {
        ...
        None if error.is::<crate::storage::source_io::Error>() => Some(
            error
                .downcast_ref::<crate::storage::source_io::Error>()
                .unwrap()
                .kind,
        ),
        None if error.is::<crate::compat::json::Error>() => Some(
            error
                .downcast_ref::<crate::compat::json::Error>()
                .unwrap()
                .kind,
        ),
...
pub fn original<'a>(
    ...
    while error.is::<crate::session::cache::SharedReadError>() {
