...
impl Claude {
    ...
    pub fn open() -> crate::Result<Self> {
        Ok(Self {
            roots: SourceRoots::new(
                || {
                    file_sessions::environment_root(
                        "CLAUDE_CONFIG_DIR",
                        ".claude",
                    )
                },
    ...
    fn files(&mut self) -> crate::Result<Vec<PathBuf>> {
        ...
        if let Some(base) = &self.roots.base {
            crate::storage::source_io::at(base, std::fs::read_dir(base))?;
        }
        Ok(files)
    }
    ...
    fn titles(
        ...
        if !self.titles.contains_key(directory) {
            let titles = Self::load_titles(
                &directory.join("sessions-index.json"),
                diagnostics,
            )?;
    ...
    fn load_titles(
        ...
        let result = (|| -> crate::Result<Value> {
            let value: Value = crate::compat::json::from_slice(
    ...
    fn parse(
        ...
        let project =
            path.parent().ok_or("Session has no project directory")?;
        let id = path
            .file_stem()
            .unwrap_or_default()
            .to_string_lossy()
            ...
            .filter(|value| truthy(value))
        {
            ...
            Some(value) => {
                let kind = crate::compat::value::type_name(value);
                return Err(format!(
                    "expected string or bytes-like object, got '{kind}'"
                )
                .into());
            }
            None => None,
        ...
        let first_user = scan
            ...
            .find_map(|record| {
                ...
                if message["role"] != "user"
                    || !crate::compat::value::truthy(&message["content"])
                {
                    return None;
                }
...
impl Provider for Claude {
    ...
    fn read(
        ...
        crate::providers::claude::transcript::read(session, diagnostics)
    }
...
mod tests {
    ...
    fn title_cache_is_per_project_and_refreshes_after_recovery() {
        ...
        let updated =
            r#"{"entries":[{"sessionId":"kept","summary":"Updated"}]}"#;
