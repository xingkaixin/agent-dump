...
pub type DiagnosticSink<'a> =
    dyn FnMut(RecoverableDiagnostic) -> crate::Result<()> + 'a;
pub type SearchRoots = Vec<(&'static str, PathBuf)>;
pub type SourceResolver =
    Box<dyn Fn() -> crate::Result<SearchRoots> + Send + Sync>;
...
impl Lookup {
    pub const fn new(session: Option<Session>) -> Self {
        Self {
            session,
            failures: Vec::new(),
        }
...
impl Discovery {
    pub const fn available(sessions: Vec<Session>) -> Self {
        Self {
            available: true,
            sessions,
            failures: Vec::new(),
...
pub trait Provider: Send + Sync {
    ...
    fn supports_format(
        &self,
        _format: crate::output::formats::OutputFormat,
    ) -> bool {
        true
    }
    ...
    fn raw_export(&self, session: &Session) -> crate::Result<RawExport> {
        ...
        if !path.is_file() {
            return Err(ProviderError::Diagnostic {
                ...
                details: vec![format!("source path: {}", crate::storage::source_io::path_text(path))],
                roots: Vec::new(),
...
pub fn source_roots(
    ...
    Ok(provider
        ...
        .into_iter()
        .map(|(label, path)| {
            format!("{label}: {}", crate::storage::source_io::path_text(&path))
        })
        .collect())
}

pub fn render_search_roots(
    info: &ProviderInfo,
