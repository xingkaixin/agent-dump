use crate::output::diagnostics::Diagnostic;
use crate::providers::contract::{Provider, source_roots};
use crate::providers::error::ProviderError;
use std::path::{Path, PathBuf};

#[allow(
    clippy::cognitive_complexity,
    reason = "This test checks the source selection contract across provider variants"
)]
pub fn source_selection<P: Provider>(
    suffix: &'static str,
    missing_root_errors: bool,
    create: impl Fn(crate::providers::files::SourceRoots) -> P,
    ...
    for initial in ["absent", "primary", "fallback", "both"] {
        for find_first in [false, true] {
            ...
            let configured = std::sync::Arc::new(std::sync::Mutex::new(
                directory.path().join("initial"),
            ));
            let fallback = directory.path().join("fallback");
            let mut provider =
                create(crate::providers::files::SourceRoots::new(
                    {
            ...
            *configured.lock().unwrap() = root.clone();
            ...
            assert!(
                !provider
                    .read(&restored, false, &mut |_| panic!(
                        "unexpected warning"
                    ))
                    .unwrap()
                    .messages
...
pub fn assert_missing(
    ...
    for zh in [false, true] {
        let rendered = Diagnostic::read_failed(
            error,
            vec!["must not replace roots".into()],
            zh,
        )
        .render(zh);
        assert!(rendered.contains(summary));
        assert!(!rendered.contains("must not replace roots"));
        assert!(rendered.contains(if zh {
            "下一步:"
        } else {
            "Next steps:"
        }));
...
pub fn removed_file(
    ...
    let path = session.source_path.as_path();
    ...
    let error = provider.raw_export(&session).err().unwrap();
    let rendered = Diagnostic::read_failed(error.as_ref(), Vec::new(), false)
        .render(false);
    assert!(
        rendered
            .contains("raw export is not supported for this session source")
    );
    assert!(
        rendered
            .contains("session source is a directory, not a single raw file")
    );
