use crate::compat::value::{field, string, text};
...
pub fn arguments(raw: &Value) -> Value {
    raw.as_str()
        .and_then(|value| crate::compat::json::from_str(value).ok())
    ...
    crate::providers::jsonl::scan_numbered(
        ...
        |seq, record, _| {
            ...
            match text(&record["role"]) {
                ...
                "assistant" => {
                    ...
                    if let Some(calls) = record["tool_calls"].as_array() {
                        for call in calls {
                            ...
                            let name =
                                field(function, "name").trim().to_owned();
                    ...
                    if !parts.is_empty() {
                        let tools_only = parts
                            .iter()
                            .all(|part| matches!(part, Part::Tool(_)));
                        let mut message =
                            Message::new(id, "assistant", 0, parts);
                ...
                "tool" => {
                    let call_id =
                        field(&record, "tool_call_id").trim().to_owned();
