use crate::providers::contract::{Discovery, Lookup, SessionFailure};
...
pub fn discover(
    ...
    let cutoff =
        Timestamp::now().checked_sub(SignedDuration::from_secs(seconds))?;
    ...
    for path in paths {
        ...
        match result {
            Ok(Some(session)) if session.created_at >= cutoff => {
            ...
            for path in paths {
                ...
                match result {
                    Ok(Some(session)) if session.id == id => {
                        if lookup.session.as_ref().is_none_or(|found| {
                            session.created_at > found.created_at
                        }) {
...
mod tests {
    ...
    fn failed_candidates_remain_visible_without_a_diagnostic_sink() {
        ...
        let discovery =
            discover(&[missing.clone(), readable], 7, true, parse).unwrap();
