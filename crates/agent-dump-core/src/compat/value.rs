...
#[allow(
    ...
    reason = "Python-compatible integer coercion truncates fractional values"
)]
...
pub fn json_object(raw: &Value) -> Option<Value> {
    crate::compat::json::from_str(raw.as_str()?)
        .ok()
        .filter(Value::is_object)
}
pub fn parse_json(raw: &Value) -> Value {
    raw.as_str()
        .and_then(|text| crate::compat::json::from_str(text).ok())
...
fn number_text(number: &serde_json::Number) -> String {
    if let Some(value) = crate::compat::json::nonfinite(number) {
...
pub fn pretty_json(value: &Value) -> String {
    fn append(value: &Value, depth: usize, out: &mut String) {
        ...
        let items: Vec<_> = match value {
            ...
            Value::Number(number) => {
                out.push_str(&if crate::compat::json::nonfinite(number)
                    .is_some()
                {
                    crate::compat::json::number_text(number)
                } else {
                    number_text(number)
                });
                return;
...
fn quoted(text: &str) -> String {
    static NONPRINTABLE: std::sync::LazyLock<regex::Regex> =
        std::sync::LazyLock::new(|| {
            regex::Regex::new(r"[\p{Other}\p{Separator}]").unwrap()
        });
    ...
    for c in text.chars() {
        match c {
            ...
            c if c != ' '
                && NONPRINTABLE.is_match(c.encode_utf8(&mut [0; 4])) =>
            {
                let code = u32::from(c);
                if code <= 255 {
...
pub fn parsed_string(value: &Value) -> Option<Value> {
    let parsed: Value =
        crate::compat::json::from_str(value.as_str()?.trim()).ok()?;
...
pub fn integer_number(value: &Value) -> serde_json::Number {
    ...
    static INTEGER: LazyLock<regex::Regex> = LazyLock::new(|| {
        regex::Regex::new(r"^[+-]?[0-9]+(?:_[0-9]+)*$").unwrap()
    });
...
pub fn decimal_digit(character: char) -> char {
    ...
    let is_decimal =
        |character: char| DECIMAL.is_match(character.encode_utf8(&mut [0; 4]));
