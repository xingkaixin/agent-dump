use crate::output::i18n::{t, terminal};
...
pub fn config_path() -> crate::Result<PathBuf> {
    ...
    crate::providers::files::environment_root(
        "HOME",
        ".config/agent-dump/config.toml",
    )
}
...
fn json_value(value: &toml_edit::Value) -> Value {
    match value {
        ...
        toml_edit::Value::Float(value) => {
            crate::compat::json::float(*value.value()).into()
        }
        toml_edit::Value::Boolean(value) => (*value.value()).into(),
        toml_edit::Value::Datetime(value) => value.value().to_string().into(),
        toml_edit::Value::Array(array) => {
            Value::Array(array.iter().map(json_value).collect())
        }
...
impl Config {
    ...
    pub fn at(path: &Path) -> crate::Result<Self> {
        ...
        let source = if exists {
            let bytes = crate::storage::source_io::read(path)?;
            std::str::from_utf8(&bytes)
                .map_err(|error| {
                    crate::compat::json::invalid_utf8(&bytes, error)
                })?
                .replace("\r\n", "\n")
                .replace('\r', "\n")
        } else {
    ...
    pub fn require_valid(&self, zh: bool) -> crate::Result<()> {
        if self.mode == ParseMode::Invalid {
            return Err(terminal(
                ...
                zh,
                &[("path", crate::storage::source_io::path_text(&self.path))],
            )
            .into());
        }
        Ok(())
    ...
    pub fn collect(&self) -> CollectConfig {
        ...
        if let Some(agents) = self.data["agent"].as_object() {
            for (name, values) in agents {
                if let Some(paths) =
                    strings(&values["deny"]).filter(|v| !v.is_empty())
                {
        ...
        CollectConfig {
            concurrency: usize::try_from(positive(
                &self.data["collect"]["summary_concurrency"],
                4,
                Some(32),
            ))
            .expect("concurrency is capped at 32"),
            timeout: positive(
                &self.data["collect"]["summary_timeout_seconds"],
                90,
                None,
            ),
            denies,
    ...
    pub fn shortcuts(&self) -> Vec<(String, Shortcut)> {
        ...
        if let Some(values) = self.data["shortcut"].as_object() {
            for (name, shortcut) in values {
                ...
                {
                    shortcuts
                        .push((name.trim().into(), Shortcut { params, args }));
                }
            }
        }
        shortcuts
    ...
    pub fn write(
        ...
        if self.mode != ParseMode::Toml {
            return Err(
                "cannot safely update a configuration that is not valid TOML"
                    .into(),
            );
        }
        ...
        crate::storage::private_files::write_text(
            &self.path,
...
fn strings(value: &Value) -> Option<Vec<String>> {
    value.as_array().map(|items| {
        items
            .iter()
            .map(|v| crate::compat::value::string(v).trim().to_owned())
            .filter(|s| !s.is_empty())
            .collect()
    })
}
fn positive(value: &Value, default: u64, maximum: Option<u64>) -> u64 {
    ...
    let value = crate::compat::value::integer_number(value).as_u64();
    value
