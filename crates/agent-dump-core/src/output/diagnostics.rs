...
use crate::output::render::safe_line;
use crate::providers::contract::{
    Provider, ProviderInfo, RecoverableDiagnostic, SessionFailure,
};
use crate::providers::error::{self as provider_error, ProviderError};
use std::fmt::Write;
...
pub fn record_warning(diagnostic: &RecoverableDiagnostic, zh: bool) -> String {
    match diagnostic {
        ...
        RecoverableDiagnostic::TitleCacheEntriesSkipped { path, count } => {
            let path = safe_line(&crate::storage::source_io::path_text(path));
            if zh {
                format!("警告: {path} 跳过了 {count} 条格式错误的标题缓存记录")
            } else {
                format!(
                    "⚠️  {path}: skipped {count} malformed title cache entries"
                )
            }
        }
        RecoverableDiagnostic::JsonlRecordsSkipped { path, count, lines } => {
            ...
            if zh {
                format!(
                    "警告: {path} 跳过了 {count} 条格式错误的记录（行 {lines}）"
                )
            } else {
                format!(
                    "⚠️  {path}: skipped {count} malformed records (lines {lines})"
                )
            }
        }
...
impl Diagnostic {
    pub fn interactive_print(zh: bool) -> Self {
        Self {
            summary: crate::output::i18n::t(
                "DIAG_PRINT_UNSUPPORTED_MODE",
                zh,
                &[],
            ),
            capability: Some(crate::output::i18n::t(
                "DIAG_PRINT_UNSUPPORTED_DETAIL",
                zh,
                &[],
            )),
            next_steps: vec![crate::output::i18n::t(
                "DIAG_STEP_DROP_PRINT",
                zh,
                &[],
            )],
            ..Self::default()
    ...
    pub fn query_error(
        ...
        Self {
            summary: crate::output::i18n::t(summary, zh, &[]),
            ...
            next_steps: steps
                .into_iter()
                .map(|key| crate::output::i18n::t(key, zh, &[]))
                .collect(),
            ..Self::default()
        }
    }
    ...
    pub fn unsupported_formats(
        ...
        Some(Self {
            summary: if zh {
                ...
                format!(
                    "The current request uses an export capability {name} does not support."
                )
            },
            capability: Some(if zh {
                ...
                format!(
                    "{name} supports only {supported}; requested {requested}"
                )
            }),
            next_steps: if zh {
                vec![
    ...
    pub fn render(&self, zh: bool) -> String {
        ...
        if let Some(uri) = &self.uri {
            ...
            .unwrap();
            for (label, value) in
                [("scheme", &uri.scheme), ("session_id", &uri.id)]
            {
                if let Some(value) = value {
                    writeln!(output, "  - {label}: {}", safe_line(value))
                        .unwrap();
                }
            }
        }
