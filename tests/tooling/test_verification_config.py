...
class TestJustLintGatesFormatting:
    def test_lint_recipe_checks_both_check_and_format(self):
        ...
        assert "lint-tools" in lint_recipe
        assert "ruff check ." in justfile
        assert "ruff format --check ." in justfile
...
class TestCiDoesNotRepeatVersionIndependentWork:
    ...
    def test_lint_and_typecheck_run_exactly_once(self):
        ...
        assert "run: just lint-tools" in quality
        assert "run: just check-tools" in quality
        assert "matrix:" not in quality, "质量检查不该跟着 matrix 展开"
        assert "run: just lint-tools" not in rust, "lint 不该在每个 Python leg 重复"
        assert "run: just check-tools" not in rust, "typecheck 不该在每个 Python leg 重复"
    ...
    def test_no_job_sets_up_a_runtime_it_does_not_use(self):
        ...
        assert "setup-node" not in self._job("rust")
