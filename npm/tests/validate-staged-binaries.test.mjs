...
test("identifies each real binary format and architecture", async () => {
...
test("accepts a correctly staged binary for every target", async () => {
  for (const target of STAGED_TARGETS) {
    const filePath = await writeFixture(FIXTURES[target]);
    await validateStagedBinary(target, filePath);
  }
});

test("rejects two swapped darwin binaries", async () => {
  const armAsIntel = await writeFixture(FIXTURES["darwin-arm64"]);
  await assert.rejects(
    validateStagedBinary("darwin-x64", armAsIntel),
    /is Mach-O\/arm64, expected Mach-O\/x64/
  );

  const intelAsArm = await writeFixture(FIXTURES["darwin-x64"]);
  await assert.rejects(
...
test("rejects a linux binary staged as the windows package", async () => {
  const filePath = await writeFixture(FIXTURES["linux-x64"]);
  await assert.rejects(validateStagedBinary("win32-x64", filePath), /is ELF\/x64, expected PE\/x64/);
});

test("rejects a wrong architecture for the same format", async () => {
  const filePath = await writeFixture(FIXTURES["linux-arm64"]);
  await assert.rejects(validateStagedBinary("linux-x64", filePath), /is ELF\/arm64, expected ELF\/x64/);
});

test("rejects an empty file", async () => {
  const filePath = await writeFixture(Buffer.alloc(0));
  await assert.rejects(validateStagedBinary("linux-x64", filePath), /is empty/);
});

test("rejects a shell script that is not a native executable", async () => {
  const filePath = await writeFixture(Buffer.from("#!/bin/sh\necho hi\n", "utf8"));
  await assert.rejects(validateStagedBinary("linux-x64", filePath), /is unknown\/unknown/);
});

test("rejects a target with no declared expectation", async () => {
  const filePath = await writeFixture(FIXTURES["linux-x64"]);
