...
| `session_time_groups.py` | 会话年龄区间的闭集定义与本地日界线分组策略 |
...
| `agents/kimi_wire.py` | Kimi wire.jsonl 事件流解析与消息组装 |
...
`discover_sessions(days)` 是 Provider Discovery 的显式扩展点，一次返回
`ProviderDiscovery(available, sessions)`。能够在同一次底层发现中确定可用性和会话列表的
...
- 中文索引须将汉字与相邻非空白文本分开，命中片段从原始语料生成；正文提取或分词规则变化时递增索引内容版本，使旧缓存自动重建。
