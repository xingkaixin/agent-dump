...
impl TextQuery {
    ...
    pub fn find(&self, fields: &[&str]) -> Option<Evidence> {
        ...
        let spans: Vec<Vec<_>> = fields
            ...
            .collect();
        if !(0..self.patterns.len()).all(|term| spans.iter().any(|field| field[term].is_some())) {
            return None;
        }
        ...
        ranked.sort_by_key(|&i| std::cmp::Reverse(spans[i].iter().filter(|s| s.is_some()).count()));
        for term in 0..self.patterns.len() {
            for &i in &ranked {
                if let Some(span) = spans[i][term] {
                    ...
                    let start = before.char_indices().rev().nth(47).map_or(0, |(i, _)| i);
                    let end = after.char_indices().nth(48).map_or(after.len(), |(i, _)| i);
