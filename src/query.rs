use crate::i18n::t;
use crate::query_text::{Mode, whitespace};
...
fn provider_name(name: &str) -> Option<String> {
    ...
    crate::registry::for_name(name).ok().map(|_| name.into())
}
...
impl Query {
    pub fn parse(raw: &str, zh: bool) -> crate::Result<Self> {
        ...
        {
            let words = words.ok_or_else(|| error("QUERY_ERROR_INVALID_SYNTAX", zh))?;
                ...
                match key.trim().to_lowercase().as_str() {
                    ...
                    "path" | "cwd" => {
                        if query.path.is_some() {
                            return Err(error("QUERY_ERROR_DUPLICATE_PATH", zh));
                        }
                        query.path = Some(path(value, zh)?);
                    }
                    "limit" => {
                        if query.limit.is_some() {
                            return Err(error("QUERY_ERROR_DUPLICATE_LIMIT", zh));
                        }
        ...
        if let Some((scope, keyword)) = raw.split_once(':') {
            let names: Vec<_> = scope
                ...
                .filter(|s| !s.is_empty())
                .collect();
            if names.len() > 1 || names.iter().any(|name| provider_name(name).is_some()) {
    ...
    pub fn from_uri(raw: &str, zh: bool) -> crate::Result<Self> {
        ...
        let unknown = fields
            .keys()
            .filter(|k| !matches!(k.as_str(), "q" | "providers" | "roles" | "limit"))
            .cloned()
            .collect::<Vec<_>>();
        if !unknown.is_empty() {
            return Err(t(
        ...
        for name in ["q", "providers", "roles", "limit"] {
            ...
            if values.len() != 1 {
                return Err(t("QUERY_ERROR_DUPLICATE_FIELD", zh, &[("field", name.into())]).into());
            }
    ...
    pub fn includes_path(&self, directory: &str) -> bool {
        ...
        !directory.trim().is_empty()
            && project_path(directory).is_ok_and(|p| p.starts_with(scope) || scope.starts_with(p))
    }
    ...
    pub fn summary(&self, zh: bool) -> String {
        ...
        {
            return crate::render::safe_line(keyword);
        }
        ...
        crate::render::safe_line(&if parts.is_empty() {
