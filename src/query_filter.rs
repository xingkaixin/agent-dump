...
use crate::scanner::ScannedProvider;
use crate::session_data::SessionDataCache;
...
pub fn select(
    ...
    let text = TextQuery::new(query.keyword.as_deref().unwrap_or(""), query.mode);
        ...
        match result {
            ...
            Err(error) => {
                let kind = if error.is::<rusqlite::Error>() {
                    "DatabaseError"
                } else if let Some(error) = error.downcast_ref::<crate::source_io::Error>() {
                    error.kind
                } else {
                    "OSError"
                };
    ...
    selection.matches.sort_by(|a, b| {
        ...
        let time = || {
            sb.updated_at
                ...
                .then_with(|| groups[a.group].info.name.cmp(groups[b.group].info.name))
...
fn indexed(
    ...
    for (g, group) in groups.iter().enumerate() {
        for (s, session) in group.sessions.iter().enumerate() {
            if query.includes_path(&session.directory) {
                positions.insert((group.info.name.to_owned(), session.id.clone()), (g, s));
            }
        }
    ...
    for result in index.search(text, &keys)? {
        if let Some(&(group, session)) = positions.get(&(result.provider, result.id)) {
    ...
    for (g, group) in groups.iter().enumerate() {
        for (s, session) in group.sessions.iter().enumerate() {
            ...
            let data = match cache.lease(
                ...
                zh,
                &mut |d| {
                    writeln!(warnings, "{}", crate::diagnostics::record_warning(&d, zh))?;
                    Ok(())
                },
            ) {
                Ok(data) => data,
            ...
            let evidence = if let Some(roles) = &query.roles {
                data.messages
                    .iter()
                    .filter(|message| roles.contains(&message.role.to_lowercase()))
                    .find_map(|message| {
                        let content = crate::transcript::message_texts(message).join("\n");
                        if text.literals.is_empty() {
                            let normalized = crate::query_text::normalize(&content);
                            let excerpt: String = normalized.chars().take(96).collect();
                ...
                text.find(&[&session.title, &crate::transcript::searchable(&data)])
