"""Shared reader for providers using the OpenCode-compatible SQLite schema."""
...
class SQLiteSessionAgent(BaseAgent, ABC):
    """Read sessions from the SQLite schema shared by OpenCode and ZCode."""
    ...
    def is_available(self) -> bool:
        """Check whether the provider database exists."""
    ...
    def _build_session_data(self, conn: sqlite3.Connection, session: Session) -> NormalizedSessionData:
        ...
        for msg_row in message_rows:
            ...
            # time/tokens/cost 都由 Provider 写入：非 dict 的 time 会让 .get() 抛，
    ...
    def export_raw_session(self, session: Session, output_dir: Path) -> Path:
        """Export raw session data for a SQLite-backed provider.

        These providers store sessions in SQLite, so raw export matches JSON export content
