...
def build_collect_handoff_prompt(
    *,
    ...
    instructions = [
        ...
        "3. 不需要 agent-dump 的 API 配置；不要读取或索要 API key，不要调用 --summary 或再次运行 --collect。",
        "   使用外部模型处理会话仍受该外部 agent 的数据传输及隐私策略约束，并不等于离线处理。",
        "4. 只处理清单中的 URI，不重新扫描或扩大范围；清单已应用日期、查询和项目排除规则。",
        ...
        "",
        "## 读取与汇总流程",
        "1. 逐条执行 collect_session.read_argv 参数数组；若工具只接受 shell 字符串，使用对应 read_command。",
        "   read_command 对应 collect_task.shell。参数位置和引号必须保留，不得拼接会话标题作为命令。",
        "2. print 是可读正文视图，会省略普通工具记录，不是无损原始数据。",
        "   若需核实工具结果，用同一 URI 将 --format print 改为 --format json，",
        "   并添加 --output 指向本任务私有临时目录，再读取命令返回的 JSON 文件；不要加 --summary。",
        "3. 长会话输出被工具截断时，将 print 的 stdout 保存到私有临时文件，按段读取到文件结尾。",
        "   也应分批处理较长的候选清单。不要用 --head 代替正文，不得把截断输出当成已完整读取。",
        "4. 每读完一条会话，先记录其目标、行动、已证实结果、决策、风险、阻塞及未完成事项，附来源 URI。",
        "   只基于证据，不根据标题猜测；区分用户要求、计划、尝试与已完成结果。",
        "5. pm 模式只在同一日期和项目内合并事实；insight 模式保留单会话归属。",
        "   按下列报告格式组织摘要，事实不得跨日期、项目或来源错配。",
        "6. 单条读取失败则记录 URI 和原因，继续处理其他会话；全部失败时停止，不生成虚构报告。",
        "   记录应读、已读、失败及截断数量；没有可靠证据证明读完时，必须标记覆盖不完整。",
        "",
        "## 最终报告格式",
        *collect_report_instructions(since_date=since_date, until_date=until_date, mode=mode),
        ...
        "缺少父目录时可创建；目标文件已存在时先征求用户确认，不得静默覆盖。",
        "写入后重新读取核验路径和内容，并向用户报告最终路径、覆盖数量及未解决的问题。",
        "若文件写入失败，明确报告失败，不能声称交付完成。完成后清理本任务创建的临时数据。",
    ]
    return compose_summary_prompt(instructions, data=data)
