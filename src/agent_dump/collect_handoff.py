...
def build_collect_handoff_prompt(
    *,
    ...
    instructions = [
        ...
        "3. print 是可读正文视图，会省略普通工具记录，不是无损原始数据。",
        ...
        "   覆盖统计应区分候选、导出成功、完整阅读、失败、未读完及无实质内容数量；不得把导出成功数充作已读数。",
        "   交付前每个候选必须有阅读结论或失败原因；不能默认放弃尚未处理的候选。部分不可读时明确标记覆盖不完整。",
        "",
        "## 摘要取舍",
        "完整阅读后，只有 /usage 等命令且无结果、问候或无实质工作内容的会话计入覆盖附录，不展开成工作事项。",
        "审批或重复转录可作为关联证据，但不重复计为实现或产出；仍保留各来源归属，不跨项目合并成果。",
        "当前这次汇总任务自身的读取、恢复和写报告过程只放在执行说明中，不作为被汇总的工作成果。",
        "不要因此排除真正开发汇总功能的会话；无法确认是否为当前任务时，只如实注明，不按标题跳过阅读。",
        "同一事实选一个主要位置展开，其余章节简短引用；无实质内容的来源在附录集中列示，避免逐条重复空话。",
