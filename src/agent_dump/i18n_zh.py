...
ZH_TRANSLATIONS = {
    ...
    Keys.CLI_EMIT_PROMPT_HELP: "生成供外部 agent 执行的 collect 提示词，不需要 API 配置；--save 指定最终报告路径",
    ...
    Keys.PROVIDERS_TABLE_HEADER: "Provider | URI | 导出格式 | 搜索路径 | 不支持",
    ...
    Keys.EMIT_PROMPT_REQUIRES_COLLECT: "❌ --emit-prompt 仅支持 --collect 模式。",
    Keys.COLLECT_ACTION_CONFLICT: "❌ --emit-prompt 不能与 --dry-run 同时使用。",
    ...
    Keys.COLLECT_DISCOVERY_INCOMPLETE_REPORT: "注意：本报告不完整。发现失败 {count} 个 Provider，遗漏会话数量未知。",
    Keys.COLLECT_INCOMPLETE_REPORT: "注意：本报告不完整。读取失败 {read_failed} 个会话，摘要失败 {summary_failed} 个会话；实际包含 {included} 个会话。",
    ...
    Keys.DIAG_URI_CAPABILITY_GAP: "当前请求了 {agent} 不支持的导出能力。",
    Keys.DIAG_URI_CAPABILITY_DETAIL: "{agent} 仅支持 {supported}；当前请求了 {requested}",
