...
use crate::collect::model::{Mode, Summary};
...
pub fn normalize(payload: &Value, mode: Mode) -> crate::Result<Summary> {
    ...
    for (key, value) in object {
        ...
        {
            return Err(format!(
                "summary field {key} must be an array of strings"
            )
            .into());
        }
    }
...
fn extract(response: &str) -> crate::Result<Value> {
    ...
    static FENCE: LazyLock<regex::Regex> = LazyLock::new(|| {
        regex::Regex::new(r"(?i)```json\s*(\{[\s\S]*?\})\s*```").unwrap()
    });
    ...
    for text in candidates {
        ...
        match parsed {
            ...
            Some(Ok(_)) => {}
            _ => {
                if let Err(error) = agent_dump_core::compat::json::from_slice(
                    text.trim().as_bytes(),
                ) {
                    let message = error.to_string();
                    if let Some((reason, location)) =
                        message.rsplit_once(": line ")
                    {
                        let detail = format!(
                            "{reason} at line {} of {}",
                            location
...
pub fn request(
    ...
    loop {
        ...
        let response = match crate::collect::llm::request(
            config,
            &current,
            timeout,
            Some(mode),
        ...
        match extract(&response).and_then(|v| normalize(&v, mode)) {
            ...
            Err(error) => {
                ...
                fields["response_preview"] =
                    crate::collect::prompts::preview(&response, 400).into();
                let normalized = response.trim();
                fields["response_tail_preview"] =
                ...
                parse_attempt += 1;
                transport_attempt = 0;
                current = crate::collect::prompts::retry(
                    prompt,
                    &response,
                    mode,
