...
pub fn syntax(text: &str, original: serde_json::Error) -> Error {
    ...
    let failure = if text.starts_with('\u{feff}') {
        ...
        parser.value().err().or_else(|| {
            parser.whitespace();
            (parser.index < parser.chars.len()).then_some(("Extra data", parser.index))
        })
    };
    ...
    fn string(&mut self) -> Result<(), Failure> {
        ...
        while let Some(character) = self.chars.get(self.index).copied() {
            ...
            match character {
                '"' => return Ok(()),
                '\\' => {
                    let Some(escape) = self.chars.get(self.index).copied() else {
                        break;
                    };
                    self.index += 1;
                    if escape == 'u' {
                        if self
                            ...
                            .is_none_or(|chars| !chars.iter().all(char::is_ascii_hexdigit))
                        {
                            return Err(("Invalid \\uXXXX escape", self.index - 1));
                        }
                        self.index += 4;
                    } else if !matches!(escape, '"' | '\\' | '/' | 'b' | 'f' | 'n' | 'r' | 't') {
                        return Err(("Invalid \\escape", self.index - 2));
                    }
                }
                character if character < ' ' => {
                    return Err(("Invalid control character at", self.index - 1));
                }
                _ => {}
            }
        }
