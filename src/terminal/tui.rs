...
const fn help(zh: bool, multiple: bool) -> &'static str {
    match (zh, multiple) {
        (true, true) => {
            "↑↓ 移动 · 空格选择 · a 全选 · i 反选 · Enter 确认 · q/Esc 取消"
        }
        (false, true) => {
            "↑↓ move · Space select · a all · i invert · Enter confirm · q/Esc cancel"
        }
...
fn select_mode(
    ...
    loop {
        terminal.draw(|frame| {
            draw_selection(
                frame, title, rows, multiple, &selected, &mut state, zh,
            );
        })?;
        ...
        match key.code {
            KeyCode::Char('y' | 'Y') if mode == ChoiceMode::Confirm => {
                return Ok(Some(vec![0]));
            }
            KeyCode::Char('n' | 'N') if mode == ChoiceMode::Confirm => {
            ...
            KeyCode::PageUp => {
                cursor = cursor.saturating_sub(
                    terminal.size()?.height.saturating_sub(4).max(1) as usize,
                );
            }
            KeyCode::PageDown => {
                cursor = (cursor
...
pub fn input(
    ...
    loop {
        terminal.draw(|frame| {
            ...
            if width > 0 && areas[1].height > 1 {
                frame.set_cursor_position((
                    areas[1].x
                        + 1
                        + u16::try_from(offset).expect(
                            "cursor offset is bounded by terminal width",
                        ),
                    areas[1].y + 1,
                ));
            }
        ...
        match event::read()? {
            Event::Paste(value) => {
                let value = agent_dump_core::output::render::safe_line(&value);
...
pub fn secret_line(prompt: &str) -> crate::Result<Option<String>> {
    ...
    let result = loop {
        match event::read()? {
            Event::Paste(value) => text
                .push_str(&agent_dump_core::output::render::safe_line(&value)),
            Event::Key(key) if key.kind != KeyEventKind::Release => {
                match key.code {
