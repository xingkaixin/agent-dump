use agent_dump_core::config::{AiConfig, Config, ParseMode};
...
pub fn read_line(
    ...
    if input.read_line(&mut line)? == 0 {
        return Err(agent_dump_core::providers::error::ProviderError::Cause {
            kind: "EOFError",
...
pub fn run(
    ...
    if action == "view" && config.exists {
        ...
        writeln!(
            ...
            agent_dump_core::output::render::safe_line(
                &agent_dump_core::storage::source_io::path_text(&logging.path)
            )
        )?;
        ...
        for (name, shortcut) in shortcuts {
            writeln!(
                ...
                agent_dump_core::output::render::safe_line(&format!(
                    ...
                    agent_dump_core::compat::value::repr(&serde_json::json!(
                        shortcut.params
                    )),
                    agent_dump_core::compat::value::repr(&serde_json::json!(
                        shortcut.args
    ...
    if let Some(ai) = &ai {
        let errors = agent_dump_core::config::validate_ai(Some(ai), true);
        if !errors.is_empty() {
            writeln!(
                out,
                "{}",
    ...
    writeln!(
        ...
        terminal(
            ...
            &[(
                "path",
                agent_dump_core::storage::source_io::path_text(&config.path)
            )]
        )
    )?;
    Ok(true)
...
fn prompt(
    ...
    let tty = crate::terminal::tui::available();
    let provider = if tty {
        out.flush()?;
        let rows =
            ["OpenAI", "Anthropic"].map(|name| crate::terminal::tui::Row {
                title: name.into(),
                detail: String::new(),
                group: String::new(),
            });
        let Some(selected) = crate::terminal::tui::select(
    ...
    .zip(defaults)
    {
        let shown = if secret {
            ...
            agent_dump_core::output::render::safe_line(default)
        };
        if tty {
            ...
            let Some(value) = crate::terminal::tui::input(
                &t(key, zh, &[]),
                &shown_default,
                secret,
                zh,
            ...
            let value = if secret && std::io::stdin().is_terminal() {
                out.flush()?;
                let Some(value) = crate::terminal::tui::secret_line(&prompt)?
                else {
                    return Ok(None);
                };
                value.trim().to_owned()
    ...
    let ai = (existing.is_some() || values[..3].iter().any(|v| !v.is_empty()))
        .then(|| AiConfig {
    ...
    let confirmed = if tty {
        out.flush()?;
        crate::terminal::tui::confirm(
            &t("CONFIG_CONFIRM_WRITE", zh, &[]),
            true,
            zh,
        )?
    } else {
