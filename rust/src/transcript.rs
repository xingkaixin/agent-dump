...
fn clean(value: &Value) -> String {
    if crate::value::truthy(value) {
        crate::value::string(value).trim().into()
    } else {
        String::new()
    }
}
...
pub fn search_value(value: &Value) -> String {
    match value {
        ...
        Value::Object(values) => format!(
            ...
            values
                .iter()
                .map(|(k, v)| format!("{}: {}", serde_json::to_string(k).unwrap(), json_value(v)))
                .collect::<Vec<_>>()
                .join(", ")
        ),
        Value::Number(n) => crate::python_json::number_text(n),
        _ => value.to_string(),
    }
}
