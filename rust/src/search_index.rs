...
use crate::query_text::{Mode, TextQuery};
use crate::session::Session;
use crate::session_data::{SessionDataCache, SessionSignal};
...
fn separate_cjk(text: &str) -> String {
    ...
    for c in text.chars() {
        if previous.is_some_and(|p| {
            (cjk(p) && !crate::query_text::whitespace(c))
                || (cjk(c) && !crate::query_text::whitespace(p))
        }) {
            output.push(' ');
        }
        output.push(c);
...
fn fts_table(query: &TextQuery) -> Option<&'static str> {
    if query.literals.is_empty() || (query.mode == Mode::Phrase && query.literals[0].contains(' '))
    {
        return None;
    }
...
fn delete(connection: &Connection, rowid: i64) -> crate::Result<()> {
    connection.execute("DELETE FROM index_state WHERE fts_rowid = ?", [rowid])?;
...
fn delete_text(connection: &Connection, rowid: i64) -> crate::Result<()> {
    ...
    connection.execute("DELETE FROM sessions_fts_trigram WHERE rowid = ?", [rowid])?;
    Ok(())
}

impl SearchIndex {
    pub fn open(sources: impl IntoIterator<Item = std::path::PathBuf>) -> crate::Result<Self> {
        let root = std::env::var_os("XDG_CACHE_HOME")
            .filter(|s| !s.is_empty())
            .map(std::path::PathBuf::from)
            .unwrap_or(crate::file_sessions::environment_root("HOME", ".cache")?);
        let path = root.join("agent-dump/search-index.db");
        let resolved = crate::query::project_path(&crate::source_io::path_text(&path))?;
        for source in sources {
            let source = crate::query::project_path(&crate::source_io::path_text(&source))?;
    ...
    pub fn at(path: &Path) -> crate::Result<Self> {
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| crate::source_io::Error::native(parent, e))?;
            #[cfg(unix)]
            {
        ...
        let expired: Vec<i64> = transaction
            .prepare("SELECT fts_rowid FROM index_state WHERE last_seen_at < ?")?
            .query_map([now() - 30.0 * 86400.0], |r| r.get(0))?
            .collect::<Result<_, _>>()?;
    ...
    pub fn update(
        ...
        let pending: Vec<_> = sessions
            .iter()
            .filter_map(|session| {
                let signature = SessionSignal::new(provider, session).signature();
                (indexed
                    .get(&session.id)
        ...
        for batch in pending.chunks(32) {
                ...
                let jobs: Vec<_> = batch
                    ...
                    .map(|(session, _)| {
                        ...
                        scope.spawn(move || {
                            ...
                            let text = cache
                                .lease(info.name, provider, session, zh, &mut |d| {
                                    diagnostics.push(d);
                                    Ok(())
                                })
                                .map(|data| crate::transcript::searchable(&data));
                            (text, diagnostics)
                        })
                    })
                    .collect();
                jobs.into_iter()
                    .map(|job| job.join().map_err(|_| "Index reader panicked".into()))
            ...
            for ((session, signature), (text, _)) in batch.iter().zip(texts) {
                ...
                if latest
                    .as_ref()
                    .is_some_and(|row| row.signature == *signature || row.observed > observed)
    ...
    fn literal_search(
        ...
        for batch in keys.chunks(400) {
            let filters = vec!["(s.agent = ? AND s.session_id = ?)"; batch.len()].join(" OR ");
            let sql = format!(
