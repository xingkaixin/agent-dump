...
pub fn run(
    ...
    let mut scan =
        crate::scanner::discover(operation.query.as_ref(), operation.days, zh, warnings)?;
    if scan.groups.is_empty() {
        let roots = crate::registry::search_roots()?;
        let names = operation
            .query
            .as_ref()
    ...
    if let Some(query) = &operation.query {
        let selection = crate::query_filter::select(&scan.groups, query, zh, warnings)?;
    ...
    let index = if scan.groups.len() == 1 {
        ...
        let rows: Vec<_> = scan
            ...
            .collect();
        let Some(index) = crate::selector::provider(&rows, zh, out, input)? else {
    ...
    if let Some(diagnostic) =
        Diagnostic::unsupported_formats(group.info, group.provider.as_ref(), &operation.formats, zh)
    {
    ...
    let selected = crate::selector::sessions(
        &group.sessions,
        group.info.scheme,
        operation.metadata,
        zh,
    ...
    for (position, &index) in selected.iter().enumerate() {
        ...
        for (f, &format) in operation.formats.iter().enumerate() {
            let base = crate::export::output_base(operation.output.as_deref(), &configured, format)
                .join(group.info.name);
            if let Ok(path) = crate::export::target(&session.id, &base, format, raw.as_ref().ok()) {
                let absolute = crate::query::project_path(&crate::source_io::path_text(&path))?;
                let normalized: String = crate::source_io::path_text(&absolute).nfc().collect();
                targets
    ...
    for (position, index) in selected.into_iter().enumerate() {
        ...
        .then(|| {
            cache.get(
                ...
                zh,
                &mut |d| {
                    writeln!(warnings, "{}", diagnostics::record_warning(&d, zh))?;
                    Ok(())
                },
            )
        });
        for (f, &format) in operation.formats.iter().enumerate() {
            if collisions.contains(&(position, f)) {
                let base =
                    crate::export::output_base(operation.output.as_deref(), &configured, format)
                        .join(group.info.name);
                let path = crate::export::target(&session.id, &base, format, raw.as_ref().ok())?;
                let error = crate::provider_error::ProviderError::Cause {
                ...
                write!(
                    ...
                    Diagnostic::read_failed(
                        &error,
                        render_search_roots(group.info, group.provider.as_ref()),
                        zh
                    )
                    .render(zh)
                )?;
            ...
            let prepared = data
                ...
                .map(|d| d.as_ref());
            match (crate::export::SessionExport {
            ...
            .write(format, &base, None)
            {
                Ok(path) => {
                    exported += 1;
                    paths.insert(crate::source_io::path_text(path.parent().unwrap()));
                    writeln!(
                        out,
                        "{}",
                        terminal(
                            ...
                            &[
                                ...
                                (
                                    "filename",
                                    crate::source_io::path_text(std::path::Path::new(
    ...
    let path = if paths.is_empty() {
        crate::source_io::path_text(
            &crate::export::output_base(
