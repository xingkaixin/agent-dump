...
fn session(connection: &Connection, path: &Path, row: &Value) -> crate::Result<Session> {
...
fn message_rows(connection: &Connection, session: &Value, full: bool) -> crate::Result<Vec<Value>> {
    let columns = if full {
        "m.*"
    } else {
    ...
    let by_id: HashMap<_, _> = records.iter().map(|row| (text(&row["id"]), row)).collect();
    ...
    while !node.is_empty() {
        ...
        if !(node == active && row["role"] == "user" && !truthy(&row["parts_count"])) {
    ...
    Ok(match kind {
        ...
        kind if kind == "dynamic-tool" || kind.starts_with("tool-") => {
            ...
            let status = match text(&part["state"]) {
                "input-streaming" | "approval-requested" | "approval-responded" => "pending",
