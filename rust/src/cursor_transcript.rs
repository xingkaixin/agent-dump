...
pub fn read(connection: &Connection, session: &Session) -> crate::Result<SessionData> {
    ...
    loop {
        ...
        let Some((_, bubble)) = frame.bubbles.peek() else {
            ...
            let id = text(&frame.session.source_metadata["composer_id"]).to_owned();
            ...
            && !memo.contains_key(id)
        {
            if let Some(composer) = composer(connection, id)?.filter(truthy) {
                let session = build_session(&frame.session.source_path, id, id, &composer);
...
impl Frame {
    ...
    fn append(
        ...
        let time = bubble_time(&bubble, self.session.created_at.as_millisecond());
...
fn completion(id: &str, composer: &Value, child: &SessionData) -> Option<Message> {
