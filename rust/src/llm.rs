...
fn error(message: String, retryable: bool) -> RequestError {
    RequestError {
        message,
        retryable,
...
pub fn request(
        ...
        && config.provider == "openai"
    {
        ...
        payload["response_format"] = json!({"type":"json_schema", "json_schema":schema(mode)});
    }
    let response = match post(config, &payload, timeout) {
        Err(error) if error.rejected_thinking && config.provider == "openai" => {
    ...
    let content =
        content.ok_or_else(|| error(format!("{provider} API response missing content"), false))?;
    let content = content
        .as_str()
        .filter(|s| !s.trim().is_empty())
        .ok_or_else(|| error(format!("{provider} API returned empty content"), false))?;
    Ok(content.into())
}

fn post(config: &AiConfig, payload: &Value, timeout: u64) -> Result<Value, RequestError> {
    let provider = if config.provider == "openai" {
        "OpenAI"
    } else {
        "Anthropic"
    ...
    for redirects in 0..=10 {
            ...
            error(
                format!(
                    ...
                    if matches!(e, ureq::Error::Timeout(_)) {
                        ...
                        crate::render::safe_line(&e.to_string())
                    }
                ),
                true,
            )
        ...
        {
            if redirects == 10 {
                return Err(error(format!("{provider} API HTTP {status}"), false));
            }
            let next = url
                .join(location)
                .map_err(|e| error(format!("{provider} API request failed: {e}"), false))?;
        ...
        let too_large = || {
            error(
                if successful {
                    ...
                    format!("{provider} API HTTP {status}: response body exceeded {limit} bytes")
                },
                false,
            )
        };
            ...
            .read_to_end(&mut bytes)
        {
            return Err(if successful {
                ...
                error(
                    format!("{provider} API HTTP {status}: response body unavailable"),
        ...
        return crate::python_json::from_slice(&bytes).map_err(|e| error(e.to_string(), false));
    }
    unreachable!()
}

pub fn summary(config: &AiConfig, prompt: &str, timeout: u64) -> crate::Result<String> {
    match request(config, prompt, timeout, None) {
        Err(error) if error.retryable => Ok(request(config, prompt, timeout, None)?),
        result => Ok(result?),
    }
}
