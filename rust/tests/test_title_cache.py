...
def language(request, monkeypatch):
    monkeypatch.setattr(i18n, "lang", request.param)
...
def test_unreadable_index_recovers_with_localized_warning(cli, language, monkeypatch, provider, damage):
    ...
    expect = importlib.import_module("locale_helpers").expect
    ...
    for args in [
        ...
        for candidate in ("python", "rust"):
            ...
            prefix = expect(Keys.WARN_TITLE_CACHE_FAILED, error="")
